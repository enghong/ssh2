...
class SFTP extends EventEmitter {
  constructor(client, chanInfo, cfg) {
    ...
    this._encoding = (typeof cfg.encoding === "string" ? cfg.encoding : "utf8");
  }
  ...
  open(path, flags_, attrs, cb) {
    ...
    /*
      ...
      ATTRS         attrs
    */
    const pathLen = Buffer.byteLength(path, this._encoding);
    let p = 9;
    ...
    buf.write(path, p += 4, pathLen, this._encoding);
  ...
  unlink(filename, cb) {
    ...
    /*
      ...
      string     filename
    */
    const fnameLen = Buffer.byteLength(filename, this._encoding);
    let p = 9;
    ...
    buf.write(filename, p += 4, fnameLen, this._encoding);
  ...
  rename(oldPath, newPath, cb) {
    ...
    /*
      ...
      string     newpath
    */
    const oldLen = Buffer.byteLength(oldPath, this._encoding);
    const newLen = Buffer.byteLength(newPath, this._encoding);
    let p = 9;
    ...
    buf.write(oldPath, p += 4, oldLen, this._encoding);
    writeUInt32BE(buf, newLen, p += oldLen);
    buf.write(newPath, p += 4, newLen, this._encoding);
  ...
  symlink(targetPath, linkPath, cb) {
    ...
    /*
      ...
      string     targetpath
    */
    const linkLen = Buffer.byteLength(linkPath, this._encoding);
    const targetLen = Buffer.byteLength(targetPath, this._encoding);
    let p = 9;
    ...
    if (this._isOpenSSH) {
      ...
      buf.write(targetPath, p += 4, targetLen, this._encoding);
      writeUInt32BE(buf, linkLen, p += targetLen);
      buf.write(linkPath, p += 4, linkLen, this._encoding);
    } else {
  ...
  ext_openssh_rename(oldPath, newPath, cb) {
    ...
    */
    ...
    buf.write('posix-rename@openssh.com', p += 4, 24, this._encoding);
  ...
  ext_openssh_statvfs(path, cb) {
    ...
    */
    ...
    buf.write('statvfs@openssh.com', p += 4, 19, this._encoding);
  ...
  ext_openssh_fstatvfs(handle, cb) {
    ...
    */
    ...
    buf.write('fstatvfs@openssh.com', p += 4, 20, this._encoding);
  ...
  ext_openssh_hardlink(oldPath, newPath, cb) {
    ...
    */
    ...
    writeUInt32BE(buf, 20, p);
    buf.write('hardlink@openssh.com', p += 4, 20, this._encoding);
  ...
  ext_openssh_fsync(handle, cb) {
    ...
    */
    ...
    buf.write('fsync@openssh.com', p += 4, 17, this.encoding);
  ...
  ext_openssh_lsetstat(path, attrs, cb) {
    ...
    */
    ...
    writeUInt32BE(buf, 20, p);
    buf.write('lsetstat@openssh.com', p += 4, 20, this._encoding);
  ...
  ext_openssh_expandPath(path, cb) {
    ...
    */
    ...
    buf.write('expand-path@openssh.com', p += 4, 23, this._encoding);
  ...
  [RESPONSE.NAME]: (sftp, payload) => {
    ...
    */
    ...
    if (count !== undefined) {
      let names = [];
      for (let i = 0; i < count; ++i) {
        const filename = bufferParser.readString(false);
        const longname = bufferParser.readString(false);
        ...
        names.push({
          filename: filename.toString(sftp._encoding),
          longname: longname.toString(sftp._encoding),
          attrs,
        });
      }
      if (names !== undefined) {
